  ...
  <div>
    ...
    <el-dialog title="提示" :visible.sync="NewFirstlevel" width="30%" :before-close="handleClose1">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="NewFirstlevel = false">取 消</el-button>
    ...
    <!-- 修改 -->
    <el-dialog title="提示" :visible.sync="dialogVisible" width="30%" :before-close="handleClose2">
...
export default {
  ...
  methods: {
    getCount () {
      this.$http.get(`modules/signRegion/rgionCount`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.num1 = data.regionTypeParent
          this.num2 = data.region
          this.num3 = data.regionParent
          this.num4 = data.regionType
        } else {
    ...
    newcertain () {
      ...
      if (this.fromsign.lv === 2 && this.formLabelAlign.name != '') {
        console.log('第二级')
        let info = new FormData()
        info.append('name', this.formLabelAlign.name)
        info.append('lv', 3)
      ...
      } else if (this.fromsign.lv === 2 && this.formLabelAlign.name === '') {
        this.$message.error('该类型下暂无数据,请先增加数据');
      }
      if (this.fromsign.lv === 3 && this.formLabelAlign.name != '') {
        console.log('第三级')
        let info = new FormData()
        info.append('name', this.formLabelAlign.name)
        info.append('lv', 4)
      ...
      } else if (this.fromsign.lv === 3 && this.formLabelAlign.name === '') {
        this.$message.error('该类型下暂无数据,请先增加数据');
      }
      ...
      this.$http.get(`modules/signRegion/listByParent`, {        params: {
        ...
        if (code === 1000) {
          ...
          if (data.length == 0) {
            ...
            if (!fromsign.children) {
              this.$set(fromsign, 'children', []);
            }
            fromsign.children = data;
          }
        } else {
          this.$message.error(res.data.message);
        }
        this.Superior = fromsign
      }).catch((err) => {
        console.log('错误信息' + err)
      })
    },
    ...
    renderContent (h, { node, data, store }) {
      return (
        <span class="custom-tree-node">
          <span>{node.label}</span>
          <span>
            <el-button size="mini" type="text" on-click={() => this.append(data)} v-if="data.lv!==4">新增</el-button>
