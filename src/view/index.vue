...
export default {
  ...
  methods: {
    ...
    determine () {
      ...
      } else if (oldcode != this.passowrd) {
        this.$message.error('旧密码与登陆密码不符请重新输入');
      } else {
        let info = {
          password: this.passowrd,
          newPassword: newcode
        }
        this.$http.post(`sys/user/update_password`, info).then(res => {
          this.$message({
            message: '修改成功，请重新登陆',
            type: 'success'
          });
    ...
    dianji1 (tab, event) {
      ...
      // console.log(this.editableTabs[tab.name])
      let str = this.editableTabs[tab.name].name
      // console.log(str.substring(str.length-2))
      let id = str.substring(str.length-2)
      console.log(id)
      this.$router.push({path:this.editableTabs[tab.name].name,query:{ids:id}})
    },
    reload () {
