...
export default {
  ...
  methods: {
    getCount () {
      this.$http.get(`modules/persona/countOrder`).then(res => {
    ...
    bh (row) {
      this.obj = {
        title: '订单详情',
        name: '/index/ikonxq.vue'+'?id='+row.id,
        id: row.id
      }
    ...
    handleEdit (index, row) {
      ...
      this.obj = {
        title: '订单画像',
        name: '/index/ikonhx.vue'+'?id='+row.id,
        id: row.id
      }
      this.$store.commit('mypush', this.obj)
      this.$store.commit('myid', row.id)
      this.$router.push('/index/ikonhx.vue?id='+row.id)
