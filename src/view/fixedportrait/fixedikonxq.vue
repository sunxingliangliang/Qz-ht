<template>
  <div>
    ...
    <el-button size="mini" v-if="code===1001||code===1003||code===1002&&status===1" @click="bohui">驳回</el-button>
    <el-button size="mini" v-if="code===1001||code===1003||code===1002&&status===1" @click="shenhe">审核</el-button>
    <el-button size="mini" v-if="code===1005||code===1001&&status===5" @click="fabu">发布</el-button>
    <!-- 驳回 -->
...
export default {
  ...
  methods: {
    getDetails () {
      this.$http.get(`pc/fixed/selectFixedById`,{params:{
        ...
        if (code === 1000) {
          ...
          let deviceList = data.decice.deviceList
    ...
    certain () {
      ...
      info.append('type', 5)
      info.append('remark', this.textarea1)
      this.$http.post(`modules/fixed/verify`, { 'taskId': this.id, 'type': 5, 'remark': this.textarea }).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          console.log(data)
          this.$message({
