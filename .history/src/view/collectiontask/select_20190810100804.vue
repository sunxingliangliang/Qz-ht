  ...
  methods: {
    ...
    adjunction (deviceList) {
      ...
      deviceList.forEach(item => {
        lng = item.lng
        lat = item.lat
        this.$jsonp(`http://api.map.baidu.com/geocoder/v2/?ak=1IGwblSXzAV0yxzCq0ZGdYoixoreCQwS&callback=renderReverse&location=${lat},${this.center.lng}&output=json&pois=1 `).then(res => {
          // var {code,data}=res.data
          this.addressname = res.result.formatted_address
        }).catch((err) => {
          console.log('错误信息' + err)
