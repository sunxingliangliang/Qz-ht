<template>
  <div>
    ...
    <el-table :data="deviceList" border style="width: 100%">
      ...
      <el-table-column prop="lwstatus" label="联网状态">
        <template slot-scope="scope">
      ...
      <el-table-column label="位置信息">
        <template slot-scope="scope">
          <span @click="renwuzb(scope.row)" :class="$style.f_renwuid" v-show="obscure">{{scope.row.lat}}\{{scope.row.lng}}</span>
          <span :class="$style.f_renwuid" v-show="obscure1">{{scope.row.addressname}}</span>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    renwuzb (row) {
      ...
      // }
      // this.adjunction()
      this.$jsonp(`http://api.map.baidu.com/geocoder/v2/?ak=1IGwblSXzAV0yxzCq0ZGdYoixoreCQwS&callback=renderReverse&location=${row.lat},${row.lng}&output=json&pois=1 `).then(res => {
        // var {code,data}=res.data
        this.addressname = res.result.formatted_address
        row.name=this.addressname
        this.obscure=false
        this.obscure1=true
        console.log(row)
      }).catch((err) => {
    ...
    adjunction () {
      this.$jsonp(`http://api.map.baidu.com/geocoder/v2/?ak=1IGwblSXzAV0yxzCq0ZGdYoixoreCQwS&callback=renderReverse&location=${this.center.lat},${this.center.lng}&output=json&pois=1 `).then(res => {
        // var {code,data}=res.data
        this.addressname = res.result.formatted_address
      }).catch((err) => {
        console.log('错误信息' + err)
