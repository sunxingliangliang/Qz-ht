    ...
    <el-row :class="$style.f_row">
      <el-col :span="4" :style="$style.f_da">
        <el-card shadow="hover">
          <span :class="$style.f_sl">{{ALL_NUM}}</span>
          <p>全部任务</p>
        </el-card>
      </el-col>
      ...
      <el-col :span="4" :class="$style.f_sheng">
        <el-card shadow="hover">
          <span :class="$style.f_sl">{{PLAT_NUM}}</span>
          <p>平台任务</p>
        </el-card>
      </el-col>
    </el-row>
    ...
    <div :class="$style.table">
      <el-table ref="filterTable" :data="tableData" style="width: 100%">
        ...
        <el-table-column prop="rwlx" label="任务类型">
          <template slot-scope="scope">
            <span v-if="scope.row.type===1">自建任务</span>
            <span v-if="scope.row.type===2">平台任务</span>
          </template>
        </el-table-column>
        <el-table-column prop="merchant_name" label="客户名称"></el-table-column>
        <el-table-column prop="from_date" label="采集时间" width="180">
        </el-table-column>
...
export default {
  ...
  methods: {
    getList () {
      this.$http.get(`modules/task/taskList`).then(res => {
    ...
    handleEdit (row) {
      ...
      if (row.rwlx === '平台任务') {
        ...
        this.$store.commit('mypush', this.obj)
        this.$router.push('/index/select.vue')
      }
    }
  }
}
