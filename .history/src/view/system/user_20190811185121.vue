<template>
  <div>
    <sx></sx>
    <el-input placeholder="请输入登录账户" v-model="user" :class="$style.name" clearable></el-input>
...
export default {
  ...
  methods: {
    ...
    search () {
      ...
      this.$http.get(`sys/user?keyword.fields=name&keyword.value=${this.user}`,{params:{`keyword.fields`:name}}).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.tableData = data.content
        }
    ...
    handleCurrentChange (val) {
      this.$http.get(`sys/user?size=${this.sizes}&page=${val - 1}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.tableData = data.content
          this.total = data.total
    ...
    handleSizeChange (val) {
      this.sizes = val
      this.sizes = val
      this.$http.get(`sys/user?size=${this.sizes}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.tableData = data.content
          this.total = data.total
