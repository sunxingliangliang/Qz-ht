...
export default {
  ...
  methods: {
    ...
    getList () {
      this.$http.post(`modules/financeAfter/customerList?size=${this.sizes}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          console.log(data)
    ...
    search () {
      ...
      this.$http.post(`modules/financeAfter/customerList?size=${this.sizes}`,info).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          console.log(data)
          this.tableData = data.content
    ...
    handleSizeChange (val) {
      this.sizes = val
      this.$http.post(`modules/financeAfter/list?size=${val}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.tableData = data.content
          this.total = data.total
    ...
    handleCurrentChange (val) {
      this.$http.post(`modules/financeAfter/list?size=${this.sizes}&page=${val - 1}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.tableData = data.content
          this.total = data.total
