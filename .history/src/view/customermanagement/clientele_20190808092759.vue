      ...
      <el-table ref="singleTable" :data="tableData" highlight-current-row style="width: 100%">
        ...
        <el-table-column property="stamp" label="类型" sortable>
          <template slot-scope="scope">
            ...
            <span v-if="scope.row.proxy_type===2">省级运营中心</span>
            <span v-if="scope.row.proxy_type===3">省级运营中心</span>
            <span v-if="scope.row.proxy_type===4">省级运营中心</span>
            <span v-if="scope.row.proxy_type===5">省级运营中心</span>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    getTotle () {
      this.$http.get(`modules/merchant/list`).then(res => {
