  ...
  <div>
    ...
    <div :class="$style.table">
      <el-table ref="singleTable" :data="tableData" highlight-current-row style="width: 100%">
        ...
        <el-table-column property="region" label="区域" sortable>
        </el-table-column>
        <el-table-column label="操作" width="400" style="text-align:center">
          <template slot-scope="scope">
            ...
            >重置密码</el-button>
            <el-button size="mini" type="warning" v-if="scope.row.status===1" @click="handleDelete(scope.$index, scope.row)">停用</el-button>
            <el-button size="mini" type="primary" v-if="scope.row.status===2" @click="handleDelete(scope.$index, scope.row)">启用</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods: {
    getTotle () {
      this.$http.get(`modules/merchant/list?size=${this.sizes}&page=${this.pages}`).then(res => {
