...
export default {
  ...
  methods: {
    getList () {
      this.$http.get(`modules/device/adminList?size=${this.sizes}&page=${this.pages}`).then(res => {
    ...
    search(){
       this.$http.get(`modules/device/adminList?size=${this.sizes}&page=${this.pages}&search=${this.input}&type=${this.value}&proxy=${this.value1}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.tableData = data.content
          this.total = data.total
