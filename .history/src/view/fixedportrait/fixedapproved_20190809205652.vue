...
export default {
  ...
  methods: {
    ...
    getList () {
      this.$http.get(`modules/fixed/adminList?size=${this.sizes}&page=${this.pages}&status=1`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
    ...
    bh (row) {
       this.obj = {
        title: '固定画像详情',
        name: '/index/fixedapprovedxq.vue',
        id: '50'
      }
      this.$store.commit('mypush', this.obj)
      this.$store.commit('myrow', row)
      this.$router.push({ path: '/index/fixedapprovedxq.vue', query: { id: row.id } })
    },
    search () {
      ...
      if (this.value1 === null) {
        console.log('1')
        this.value1 = ''
        this.$http.get(`modules/fixed/adminList?createTime=${this.value1}&search=${this.input}&proxy=${this.value2}&status=1&size=${this.sizes}&page=${this.pages}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            this.tableData = data.content
            this.total = data.total
        ...
        this.$http.get(`modules/fixed/adminList?createTime=${this.value1}&search=${this.input}&proxy=${this.value2}&status=${this.value}&size=${this.sizes}&page=${this.pages}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            this.tableData = data.content
            this.total = data.total
    ...
    handleSizeChange (val) {
      this.sizes = val
      this.$http.get(`modules/fixed/adminList?size=${val}&page=${this.pages}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          console.log(data)
          this.tableData = data.content
    ...
    handleCurrentChange (val) {
      console.log(val)
      this.pages = val
      this.$http.get(`modules/fixed/adminList?size=${this.sizes}&page=${val - 1}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          console.log(data)
          this.tableData = data.content
