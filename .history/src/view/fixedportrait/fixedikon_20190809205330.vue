  ...
  methods: {
    ...
    search () {
      ...
      if (this.value1 === null) {
        ...
        this.$http.get(`modules/fixed/adminList?createTime=${this.value1}&search=${this.input}&prproxyoxyType=${this.value2}&status=${this.value}&size=${this.sizes}&page=${this.pages}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            this.tableData = data.content
            this.total = data.total
