...
export default {
  ...
  methods: {
    ...
    getList () {
      this.$http.get(`modules/persona?size=${this.sizes}&page=${this.pages}&status=1`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
    ...
    search () {
      if (this.value1 === null) {
        this.value1 = ''
        this.$http.get(`modules/persona?search=${this.input}&size=${this.sizes}&proxyType=${this.value2}&createTime=${this.value1}&status=1`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            this.tableData = data.content
            this.total = data.total
        ...
        this.$http.get(`modules/persona?search=${this.input}&size=${this.sizes}&proxyType=${this.value2}&createTime=${this.value1}`).then(res => {
          var { code, data } = res.data
          if (code === 1000) {
            this.tableData = data.content
            this.total = data.total
