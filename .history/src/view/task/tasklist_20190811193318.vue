...
export default {
  ...
  methods: {
    ...
    getList () {
      this.$http.get(`pc/platform/list?size=10`, {
        params: {
          size: this.sizes,
          page: this.pages
        }
    ...
    handleCurrentChange (val) {
      this.$http.get(`pc/platform/list?size=${this.sizes}&page=${val - 1}`, {
        params: {
          size: this.sizes,
          page: val - 1
        }
    ...
    search () {
      this.$http.get(`pc/platform/list?search=${this.input}&status=${this.value}`).then(res => {
        var { code, data } = res.data
        if (code === 1000) {
          this.tableData = data.content
          this.tableData.forEach(item => {
