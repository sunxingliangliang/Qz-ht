    ...
    <div>
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column label="奖励金额">
          <el-table-column prop="yjlje" label="已奖励金额"></el-table-column>
      ...
      this.$http.get(`pc/platform/list?size=10`).then(res => {
        ...
        if (code === 1000) {
          ...
          this.tableData.forEach(item => {
            ...
            // item.yjlje = Number(item.price)*Number(item.data_num)
            // item.zje = Number(item.price) *Number(item.amount_count)
            console.log(item.wcjd)
            console.log(Number(item.data_num))
          })
