...
export default {
  ...
  methods: {
    go () {
      ...
      let md = md5(this.formLabelAlign.passowrd);
      // console.log(md)
      if (this.confirmWords === '验证通过') {
        this.$http.post(`sys/user/login`, { username: username, password: md }).then(res => {
          ...
          if (code === 1000) {
            ...
            if (data.identity.code == 1001) {
              ...
              window.sessionStorage.setItem('info', JSON.stringify(mima))
